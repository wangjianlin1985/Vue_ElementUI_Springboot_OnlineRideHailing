<template>
  <el-breadcrumb class="app-breadcrumb" separator="/" style="height:40px;backgroundColor:rgba(255, 255, 255, 1);borderRadius:20px;padding:0px 20px 0px 20px;boxShadow:0px 6px 6px #F5B61B;borderWidth:5px;borderStyle:solid;borderColor:rgba(245, 182, 27, 1);">
    <transition-group name="breadcrumb" class="box" :style="1==1?'justifyContent:flex-start;':1==2?'justifyContent:center;':'justifyContent:flex-end;'">
      <el-breadcrumb-item v-for="(item,index) in levelList" :key="item.path">
        <span v-if="item.redirect==='noRedirect'||index==levelList.length-1" class="no-redirect" style="color:rgba(245, 182, 27, 1)">{{ item.name }}</span>
    ...
    breadcrumbStyleChange(val) {
      this.$nextTick(()=>{
        document.querySelectorAll('.app-breadcrumb .el-breadcrumb__separator').forEach(el=>{
          el.innerText = "/"
          el.style.color = "rgba(245, 182, 27, 1)"
        })
